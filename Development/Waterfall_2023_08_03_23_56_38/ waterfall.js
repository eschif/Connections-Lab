...
//load all the images using a for loop that sifts through an array of file names
function preload(){
    imageNames = loadStrings(imageFolder, function loadSprites (){
        for(let i =0; i<imageNames.length; i++){
            waterSprites[i]=loadImage(imageFolder + "/"+imageNames[i]);
        }
    })  
}
 ...
 //drawSprite(initialX+10, -30,'warrior.png',.06,10);
  if (counter % 20 == 0){
    for (let i = 0; i < waterSprites.length; i++){
     drawSprite(initialX + i*random(10,15), -30, imageNames[i], .08, 10);
  warrior = drawSprite(initialX-50, -30,'warrior.png',.06,10);
...
// //   this.x = initialX + (random(-40,40));
//   sprite.img = name;
//   //sprite.img = random(waterSprites);
//   sprite.scale = scale;
